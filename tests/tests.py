...
from src.entities import TestTable
...
class BasicTests(unittest.TestCase):
    ...
    def build_random_item(self, item: TestTable):
        item.string_field = self.random_string(10)
        item.string_field2 = self.random_string(10)
        item.int_field = random.randint(0, 100)
        item.float_field = random.uniform(0, 100)
        item.bool_field = random.randint(0, 2) == 1
        item.date_time_field = datetime.now()

    def create_random_item(self) -> TestTable:
        item = TestTable()
